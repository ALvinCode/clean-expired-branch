...
class TagCleaner {
  ...
  normalizeError(errorMessage) {
    ...
    // 处理服务器拒绝错误
    if (normalized.includes('remote rejected') || normalized.includes('remote: error')) {
      return '服务器拒绝删除';
    }
    ...
    if (normalized.includes('Command failed')) {
      // 提取第一行作为主要错误信息，但保留更多细节
      const firstLine = normalized.split('\n')[0];
      // 移除具体的标签名，但保留其他错误信息
      const cleaned = firstLine.replace(/git push.*?--delete\s+"[^"]*"/, 'git push --delete');
      return cleaned.length > 150 ? cleaned.substring(0, 150) + '...' : cleaned;
    }
    
    // 默认返回前150个字符，保留更多错误信息
  ...
  async cleanLocalTags(tags) {
    ...
    const progressBar = new cliProgress.SingleBar({
      format: '🏷️  本地标签 |{bar}| {percentage}% | {value}/{total} | 成功: {success} | 失败: {failed}',
      barCompleteChar: '\u2588',
      barIncompleteChar: '\u2591',
      hideCursor: true,
